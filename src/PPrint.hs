...
ty2doc (STVar x) = typeColor (pretty x)
...
t2doc at (SLam p ((v,ty):xs) t) =
  parenIf at $
  sep [sep [ keywordColor (pretty "fun")
           , binding2doc (v,ty)
           , opColor(pretty "->")]
      , nest 2 (t2doc False (SLam p xs t))]
...
t2doc at (SFix p (f,fty) ((x,xty):xs) m) =
  parenIf at $
  sep [ sep [keywordColor (pretty "fix")
                  , binding2doc (f, fty)
                  , binding2doc (x, xty)
                  , opColor (pretty "->") ]
      , nest 2 (t2doc False (SLam p xs m))
      ]
