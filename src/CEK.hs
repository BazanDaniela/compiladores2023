module CEK (seek, val2TermCEK) where
...
seek (V _ (Bound i)) e k = case lookupEnv e i of
                            Nothing -> abort "Error de ejecución: variable encontrada."
                            Just v -> destroy v k
...
seek (Lam _ f fty t) e k =  destroy (Cl (ClosFun e f t)) k
seek (Fix _ f fty x xty t) e k = destroy (Cl (ClosFix e f x t)) k
seek (Let _ x xty t u) e k = seek t e ((KLet e u):k)
...
destroy (N n') ((KValOp op (N n)): k) = destroy (N (semOp n n')) k
